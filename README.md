# From VS Code to AstroNvim: A Lighter Workflow for Coding with Agents

*Edit in Neovim, run agents in a split terminal, ship with Lazygit.*

---

## Introduction

I have been researching a migration away from traditional IDEs — VS Code and similar editors — toward a simpler stack built around **AstroNvim**, **toggleterm**, and **Lazygit**.

The goal is practical: focus on agent commands in a terminal split beside my code, make small edits directly in AstroNvim, and push changes fast without leaving the editor. A lighter setup like AstroNvim can also use fewer resources than a full IDE with many extensions running in the background.

If you want to understand why this workflow matters and how to apply it yourself, this article walks through the research — setup, daily commands, the agent loop, Git — and the small Lua customizations I added on top of the AstroNvim template.

Throughout this article, **`<Space>` means the Leader key** (configured as Space in AstroNvim). Press Space, release, then press the next key — e.g. `<Space> e` is Space, then `e`.
...
Neovim behavior depends on which **mode** you are in. Most shortcuts below require **Normal mode** (press `Esc` to return to it).
...
This is the centerpiece of the agent workflow — a persistent shell split inside Neovim.
...
Start in your project root, open the file tree, launch a horizontal terminal for your agent, switch focus between panes to review changes, reload buffers when needed, then stage and push through Lazygit — that is the full loop this research is built around.
...
AstroNvim stores user config under `~/.config/nvim/lua/`. I started from the [AstroNvim](https://github.com/AstroNvim/AstroNvim) v6+ template and kept changes minimal — small tweaks that make daily use comfortable, especially when reviewing agent-driven diffs.
...
When an agent modifies several files, these simple gutter signs make it easy to spot what changed at a glance.
...
This pairs directly with the agent workflow: when output streams in, the terminal does not force-scroll to the bottom, so you can read earlier responses without fighting the buffer.

---

## Conclusion

This research is about a focused migration — from a heavy IDE to AstroNvim with toggleterm and Lazygit — so you can run agents beside your code, edit in place, and ship changes quickly. AstroNvim handles the hard parts (LSP, file tree, search, Git signs); toggleterm keeps the agent in view; Lazygit closes the loop from diff to push.

You do not need a perfect config on day one. Start with the official template, learn the agent loop, and customize incrementally — the way I did with a dark theme, gutter signs, and a terminal that does not auto-scroll.

If you try this workflow, **share your thoughts with me** — what worked, what did not, which agents you use in the split terminal, and what you changed in your own `lua/` folder. I would love to hear from you in the comments.

---

*Suggested Medium tags: `Neovim` · `Developer Tools` · `Productivity` · `Git` · `Software Development`*
